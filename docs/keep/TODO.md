...
|---|---|---|---|
| **Test `keep-up.sh` from nothing, in one pass** | The same on a clean machine, in a single uninterrupted run (I fixed gaps between runs, so no single pass from an empty VM has been done), plus a real Mac connected through Solvor | On a clean Ubuntu 24.04 VM (nested KVM) every phase now works, but only after fixes made along the way ([VERIFICATION.md](VERIFICATION.md)); FluxVM's AppArmor profile still blocks the image build unless `--apparmor-complain` is given ([zyvorai/fluxvm#107](https://github.com/zyvorai/fluxvm/issues/107)) | `sudo ./scripts/keep-up.sh --install-fluxvm` ends with a token in one pass, Solvor connected, `demos-ci.sh` green |
