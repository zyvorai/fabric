        ...
        {profiles.map(p => (
          <div key={p.name} className="bg-gray-900 rounded-lg p-6 border border-gray-800 hover:border-gray-800 transition">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-400 flex items-center gap-1"><Cpu className="w-3 h-3" />CPUs</span><span className="font-medium">{p.cpus}</span></div>
              <div className="flex justify-between"><span className="text-gray-400 flex items-center gap-1"><MemoryStick className="w-3 h-3" />Memory</span><span className="font-medium">{p.memory >= 1024 ? `${p.memory / 1024} GB` : `${p.memory} MB`}</span></div>
              <div className="flex justify-between"><span className="text-gray-400 flex items-center gap-1"><HardDrive className="w-3 h-3" />Disk</span><span className="font-medium">{p.disk} GB</span></div>
              {p.network_bandwidth && <div className="flex justify-between"><span className="text-gray-400">Network</span><span className="font-medium">{p.network_bandwidth}</span></div>}
            </div>
            {p.builtin && <div className="mt-3 text-xs text-gray-500">Built-in</div>}
          </div>
        ))}
      </div>
