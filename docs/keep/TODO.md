...
| **A host set up for agent sessions** | A Keep host with FluxVM tap+netns networking (or `ZYVOR_AGENT_EGRESS_ADVERTISE_HOST` set to an address the guest can reach). The lab host is shared and set up only for use cells. The cell image now brings its NIC up with DHCP and `dnsmasq-base` is installed there, so guests get an address and a route, but FluxVM's host-to-guest HTTP proxy to the worker port still fails in netns mode (`ip netns exec` returns EINVAL on kernel 7.0), so no run has completed yet | A successful `/v1/agui` run on a real cell, and any other agent session (`/v1/sessions`) on that host. Today AG-UI is verified with a stub cell and, on the lab host, only its error path | `echo-agent` streams `RUN_STARTED`, a reply and `RUN_FINISHED` on a real cell |
