...
| **A host set up for agent sessions** | A Keep host with FluxVM tap+netns networking (or `ZYVOR_AGENT_EGRESS_ADVERTISE_HOST` set to an address the guest can reach). The lab host is shared and set up only for use cells, so I did not change its networking | A successful `/v1/agui` run on a real cell, and any other agent session (`/v1/sessions`) on that host. Today AG-UI is verified with a stub cell and, on the lab host, only its error path | `echo-agent` streams `RUN_STARTED`, a reply and `RUN_FINISHED` on a real cell |
