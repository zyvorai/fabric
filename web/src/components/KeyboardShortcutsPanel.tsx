...
export default function KeyboardShortcutsPanel() {
  ...
  return (
    ...
    >
      <div
        className="bg-gray-900 rounded-xl shadow-2xl border border-gray-800 w-full max-w-lg overflow-hidden"
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          ...
          <button onClick={() => setIsOpen(false)} className="p-1.5 rounded-md text-gray-500 hover:text-white hover:bg-white/5 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        {shortcut.keys.map((key, ki) => (
                          <span key={ki} className="flex items-center gap-1">
                            <kbd className="px-2 py-0.5 bg-gray-800 border border-gray-700 rounded text-xs font-mono text-gray-300 min-w-[24px] text-center">
                              {key}
                            </kbd>
                            {ki < shortcut.keys.length - 1 && (
                              <span className="text-[10px] text-gray-600">then</span>
                            )}
                          </span>
                        ))}
                      </div>
        ...
        <div className="px-5 py-3 border-t border-gray-800 text-center text-[11px] text-gray-600">
          Press <kbd className="px-1.5 py-0.5 bg-gray-800 border border-gray-700 rounded font-mono mx-0.5">?</kbd> to toggle
        </div>
      </div>
    </div>
  )
