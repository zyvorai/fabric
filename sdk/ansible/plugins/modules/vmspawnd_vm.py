...
---
module: vmspawnd_vm
short_description: Manage VMs in vmspawnd
description:
    - Create, delete, start, and stop virtual machines managed by vmspawnd.
...
options:
    ...
    tags:
        ...
        elements: str
    api_url:
        description: URL of the vmspawnd API server.
        type: str
...
- name: Create a VM
  vmspawnd_vm:
    name: web-01
    state: present
    image: /var/lib/vmspawnd/images/ubuntu.img
    cpus: 4
    memory: 4096
    disk: 40
...
class VmspawndAPI:
    """Minimal API wrapper for the vmspawnd REST API."""
...
def run_module():
    ...
    try:
        ...
        module.fail_json(
            msg=f"Cannot connect to vmspawnd at {api_url}. "
    ...
    try:
        ...
        module.fail_json(
            msg=f"vmspawnd API error: {error_detail}",
            exception=traceback.format_exc(),
        )
        return
    except requests.exceptions.ConnectionError:
        module.fail_json(
            msg=f"Lost connection to vmspawnd at {api_url}.",
            exception=traceback.format_exc(),
        )
        return
