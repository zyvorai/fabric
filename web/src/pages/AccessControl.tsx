...
export default function AccessControl() {
  ...
  const handleDelete = async (id: string, username: string) => {
    ...
    try { await apiFetch(`/api/users/${id}`, { method: 'DELETE' }); setUsers(prev => prev.filter(u => u.id !== id)) } catch { /* ignore */ }
  }
              ...
              {users.map(user => (
                <tr key={user.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <button onClick={() => handleToggle(user.id, user.enabled)} className="flex items-center gap-1.5">
                      <div className={`relative w-8 h-4 rounded-full transition-colors ${user.enabled ? 'bg-green-600' : 'bg-slate-600'}`}><div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${user.enabled ? 'translate-x-4' : 'translate-x-0.5'}`} /></div>
