...
fn ensure_image_in_machines(name: &str) -> Result<()> {
    ...
    let external_candidates = [
        ...
        format!("/var/lib/zyvor-fabricd/images/{}_1.raw", name),
...
/// Only allows paths under /var/lib/machines and /var/lib/zyvor-fabricd.
fn resolve_image_path(image: &str, name: &str) -> String {
    ...
    let candidates = [
        ...
        format!("/var/lib/zyvor-fabricd/images/{}", image),
        format!("/var/lib/zyvor-fabricd/images/{}.raw", name),
        format!("/var/lib/zyvor-fabricd/images/{}.qcow2", name),
    ];
...
pub fn pause_vm(name: &str) -> Result<()> {
    let cgroup = zyvor_fabric_cgroup::CgroupPath::for_machine(name);
    if cgroup.exists() {
...
fn read_cpu_usage(cgroup: &zyvor_fabric_cgroup::CgroupPath) -> Result<f64> {
...
fn read_memory_usage(cgroup: &zyvor_fabric_cgroup::CgroupPath) -> Result<u64> {
    let mem = zyvor_fabric_cgroup::MemoryController::new(cgroup.path().to_path_buf());
    mem.get_current().map_err(|e| anyhow!("{}", e))
}

fn read_disk_io(cgroup: &zyvor_fabric_cgroup::CgroupPath) -> Result<(u64, u64)> {
