...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Divider()
            Button("Read email from browser…") { NSApp.activate(ignoringOtherApps: true); openWindow(id: "main"); app.sheet = .email }.disabled(!app.connected)
            Button("Talk to Solvor…") { NSApp.activate(ignoringOtherApps: true); openWindow(id: "main"); app.sheet = .voice }.disabled(!app.connected)
            ...
            Button("Open Solvor") { NSApp.activate(ignoringOtherApps: true); openWindow(id: "main") }
