...
function MonitorTabContent({ policies, metrics, alerts, onDelete, onAcknowledge, onCreate, onSync }: MonitorTabProps) {
  ...
  return (
    <div className="bg-gray-800 rounded-lg border border-gray-700">
      ...
      {view === 'policies' && (
        policies.length === 0 ? (
          <div className="p-12 text-center text-gray-400">No monitor policies configured. Create one to monitor VM network metrics.</div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="text-left p-4 font-medium text-gray-300">Thresholds</th>
                  <th className="text-left p-4 font-medium text-gray-300">Interval</th>
              ...
              <tbody className="divide-y divide-gray-700">
                {policies.map(p => (
                  <tr key={p.id} className="hover:bg-gray-700 transition">
                    ...
                    <td className="p-4 font-mono text-sm text-gray-400">{p.interval_seconds}s</td>
                    <td className="p-4">
      ...
      {view === 'metrics' && (
        metrics.length === 0 ? (
          <div className="p-12 text-center text-gray-400">No metrics available.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  <th className="text-left p-4 font-medium text-gray-300">VM</th>
                  <th className="text-left p-4 font-medium text-gray-300">Interface</th>
                  <th className="text-left p-4 font-medium text-gray-300">RX</th>
                  <th className="text-left p-4 font-medium text-gray-300">TX</th>
                  <th className="text-left p-4 font-medium text-gray-300">RX Pkts</th>
                  <th className="text-left p-4 font-medium text-gray-300">TX Pkts</th>
                  <th className="text-left p-4 font-medium text-gray-300">Errors</th>
              ...
              <tbody className="divide-y divide-gray-700">
                {metrics.map((m, i) => (
                  <tr key={i} className="hover:bg-gray-700 transition">
                    ...
                    <td className="p-4 font-mono text-sm text-gray-400">{m.rx_packets}</td>
                    <td className="p-4 font-mono text-sm text-gray-400">{m.tx_packets}</td>
                    ...
                    <td className="p-4 text-xs text-gray-500">{new Date(m.timestamp).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {view === 'alerts' && (
        alerts.length === 0 ? (
          <div className="p-12 text-center text-gray-400">No bandwidth alerts.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  <th className="text-left p-4 font-medium text-gray-300">VM</th>
                  <th className="text-left p-4 font-medium text-gray-300">Metric</th>
                  <th className="text-left p-4 font-medium text-gray-300">Value</th>
                  <th className="text-left p-4 font-medium text-gray-300">Threshold</th>
                  <th className="text-left p-4 font-medium text-gray-300">Severity</th>
              ...
              <tbody className="divide-y divide-gray-700">
                {alerts.map(a => (
                  <tr key={a.id} className={`hover:bg-gray-700 transition ${a.acknowledged ? 'opacity-50' : ''}`}>
                    <td className="p-4 font-medium">{a.vm_name}</td>
                    <td className="p-4 font-mono text-sm text-gray-400">{a.metric}</td>
                    <td className="p-4 font-mono text-sm text-red-400">{a.value}</td>
                    <td className="p-4 font-mono text-sm text-gray-400">{a.threshold}</td>
                    <td className="p-4">
                      <StatusBadge
                        status={a.severity}
                    ...
                    <td className="p-4 text-xs text-gray-500">{new Date(a.created).toLocaleString()}</td>
                    <td className="p-4">
                      {!a.acknowledged && (
    ...
    <ModalWrapper title="Create Monitor Policy" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="border border-gray-700 rounded-lg p-4 space-y-3">
          <div className="text-sm font-medium text-gray-300">Add Threshold</div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs text-gray-400 mb-1">Metric</label>
              <select value={thMetric} onChange={e => setThMetric(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              ...
              <select value={thUnit} onChange={e => setThUnit(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500">
            ...
            <div>
              ...
              <select value={thDirection} onChange={e => setThDirection(e.target.value as MetricDirection)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500">
            ...
            <div>
              ...
              <select value={thSeverity} onChange={e => setThSeverity(e.target.value as AlertSeverity)} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500">
            ...
            <div className="space-y-1 mt-2">
              {thresholds.map((t, i) => (
                <div key={i} className="flex items-center gap-2 text-xs bg-gray-700 rounded px-2 py-1">
                  ...
                  <span className="text-gray-400">{t.value} {t.unit}</span>
