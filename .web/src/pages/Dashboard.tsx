...
export default function Dashboard() {
  ...
  const loadVMs = useCallback(async () => {
    try { setVMs(await listVMs()) } catch (err) { console.error('Failed to load VMs:', err); toast.error('Failed to load virtual machines') } finally { setLoading(false) }
  }, [toast])
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Virtual machine infrastructure overview</p>
      </div>
