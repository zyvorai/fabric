...
- **Screencast:** `WS /v1/sessions/{id}/browser/screencast` — `Page.startScreencast`
  frames only (`type:frame`); operator may send `stop` / `ack` / `ping`. Input
  takeover is refused. Auth: `Authorization: Bearer` or `?token=` (browsers).
